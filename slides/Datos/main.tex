...
\begin{itemize}
    ...
    \item El conjunto \textit{test} incluye 10.286 viviendas ubicadas solo en Chapinero, sin precio.
    \item Se añadieron variables geoespaciales de \textbf{Datos Abiertos Bogotá}: cercanía a equipamientos, estrato, avalúo catastral, criminalidad, densidad comercial.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Se unificó el título y la descripción en un solo texto por aviso.
    \item Limpieza: minúsculas, eliminación de acentos, puntuación y espacios redundantes.
    \item Revisión manual para identificar patrones sobre amenities, tamaño y condiciones del inmueble.
...
\begin{itemize}
    \item Con información textual se imputaron valores faltantes de área y número de baños.
    \item Solo se imputaron faltantes cuando el texto permitía una estimación confiable.
    \item Para valores aún faltantes: imputación por promedio condicional usando:
    \begin{itemize}
        \item barrio/UPZ, habitaciones, baños, estrato, tipo de propiedad.
    \end{itemize}
    \item Áreas agrupadas en intervalos de 10 m\textsuperscript{2} para evitar fragmentación.
    \item Este módulo mejoró medición de variables clave como área, baños y amenities.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Train y test unificados en un objeto \texttt{sf} usando coordenadas en WGS84 (EPSG 4326).
    ...
    \begin{itemize}
        ...
        \item Bibliotecas, museos, colegios, estaciones de TransMilenio.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Estrato asignado por manzana más cercana.
    \end{itemize}
    \item Delitos por localidad (2019, 2020, 2022):
    \begin{itemize}
        \item Homicidios, lesiones, hurtos, delitos sexuales, violencia intrafamiliar.
    \end{itemize}
    \item Todas las capas se reproyectaron, depuraron y renombraron para consistencia.
...
\begin{itemize}
    \item Aunque se generaron muchas variables, usarlas todas aumenta riesgo de \textit{overfitting}.
    \item Se aplicó un \textbf{modelo Lasso} para seleccionar predictores:
    \begin{itemize}
        \item Penalización L1 elimina variables con bajo poder explicativo.
        \item Reduce costos computacionales en modelos no lineales.
    \end{itemize}
...
\tiny  % <-- tamaño más pequeño para que ambas quepan

% ===== TABLA TRAIN =====
\begin{table}[ht]
\centering
\caption{Descriptivas (Train)}
\renewcommand{\arraystretch}{0.8} % compactar filas
\resizebox{\textwidth}{!}{
...
Dist. biblio & 38,605 & 592.5 & 321.7 & 4.7 & 1,873.9 \\
...
Dist. biblio & 10,286 & 404.4 & 219.3 & 7.9 & 1,598.2 \\
...
%-----------------------------------------------
%% -------------------------------------------------------
\begin{frame}{Histograma de los Precios}
\centering

\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{price_hist.png} 
    \caption{Histograma de los precios en los datos de entrenamiento. Mil Millones de COP}
\end{figure}
